...
public sealed class SpawnedPlayer : SpawnedBionic
{
    ...
    public void Deserialize(Packet packet)
    {
        ...
        if (LegacyGame.ClientType > GameClientType.Japanese_Old)
        ...
        if (LegacyGame.ClientType == GameClientType.Taiwan)
            packet.ReadUInt();
        ...
        for (var i = 0; i < itemCount; i++)
        {
            ...
            var itemObj = LegacyGame.ReferenceManager.GetRefItem(itemId);
        ...
        if (hasMask)
        {
            ...
            var maskObj = LegacyGame.ReferenceManager.GetRefObjCommon(maskId);
            if (maskObj == null)
            {
        ...
        if (LegacyGame.ClientType >= GameClientType.Chinese_Old && WearsJobSuite)
            if (WearsJobSuite)
            {
        ...
        if (LegacyGame.ClientType < GameClientType.Chinese_Old)
        {
            JobLevel = packet.ReadByte();
        ...
        if (LegacyGame.ClientType >= GameClientType.Chinese && InteractMode == InteractMode.P2N_TALK2)
            Stall = SpawnedPlayerStall.FromPacket(packet);
        else if (LegacyGame.ClientType < GameClientType.Chinese && InteractMode == InteractMode.P2N_TALK)
            Stall = SpawnedPlayerStall.FromPacket(packet);
        ...
        if (LegacyGame.ClientType >= GameClientType.Chinese && LegacyGame.ClientType < GameClientType.Rigid)
