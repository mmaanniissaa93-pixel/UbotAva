...
public class Cos : SpawnedEntity
{
    ...
    /// </value>
    public virtual long MaxExperience => LegacyGame.ReferenceManager.GetRefLevel(Level).Exp_C;
    ...
    {
        var inventory = (InventoryItemCollection)LegacyGame.Player.Inventory;
        var usingItem = inventory.GetItem(p => p.Record.IsCosHpPotion);
        if (usingItem == null)
            return false;
    ...
    public void MoveTo(Position destination, bool sleep = true)
    {
        ...
        if (sleep)
        {
            var vehicle = LegacyGame.Player.Vehicle;
            ...
            {
                Thread.Sleep(Convert.ToInt32(distance / LegacyGame.Player.ActualSpeed * 10000));
            }
        }
    }
    ...
    {
        var npc = LegacyGame.SelectedEntity;
        if (npc == null)
        {
    ...
    public bool SellItemToNpc(byte slot)
    {
        ...
        var entity = LegacyGame.SelectedEntity;
        if (entity == null)
            return false;
