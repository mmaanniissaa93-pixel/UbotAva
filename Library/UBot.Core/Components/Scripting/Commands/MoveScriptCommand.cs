...
internal class MoveScriptCommand : IScriptCommand
{
    ...
    private bool ExecuteMove(IReadOnlyList<string> arguments)
    {
        ...
        if (PlayerConfig.Get("UBot.Training.checkUseSpeedDrug", true))
        {
            if (
        ...
        if (PlayerConfig.Get("UBot.Training.checkUseMount", true))
        {
            if (
                !Game.Player.HasActiveVehicle
                && !Game.Player.InAction
                && !(
                    ...
                    && ScriptManager.File == PlayerConfig.Get("UBot.Lure.SelectedScriptPath", string.Empty)
                )
            )
            {
