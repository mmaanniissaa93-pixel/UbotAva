...
public static class ShoppingManager
{
    ...
    public static void LoadFilters()
    {
        ...
        var configSell = PlayerConfig.GetArray<string>("UBot.Shopping.Sell");
        var configStore = PlayerConfig.GetArray<string>("UBot.Shopping.Store");

        foreach (var item in configSell.Where(i => !string.IsNullOrWhiteSpace(i)).Distinct())
            SellFilter.Add(item);

        foreach (var item in configStore.Where(i => !string.IsNullOrWhiteSpace(i)).Distinct())
            StoreFilter.Add(item);
    }

    public static void SaveFilters()
    {
        PlayerConfig.SetArray("UBot.Shopping.Sell", SellFilter);
        PlayerConfig.SetArray("UBot.Shopping.Store", StoreFilter);
    }
