...
internal sealed partial class UbotPluginModuleService : UbotServiceBase
{
    ...
    internal UbotPluginModuleService(
        UbotConnectionService connectionService,
        UbotMapService mapService,
        UbotCommandCenterService commandCenterService,
        UbotAutoLoginService autoLoginService)
    {
    ...
    public Task<Dictionary<string, object?>> GetPluginConfigAsync(string pluginId)
    {
        ...
        if (TryResolvePlugin(pluginId, out var plugin))
        {
            ...
            if (IsCommandCenterPlugin(plugin))
                return Task.FromResult(_commandCenterService.BuildCommandCenterPluginConfig());
        }
    ...
    public Task<bool> SetPluginConfigAsync(string pluginId, Dictionary<string, object?> patch)
    {
        ...
        else if (TryResolvePlugin(pluginId, out var plugin))
        {
            if (IsGeneralPlugin(plugin))
                ...
                changed = _commandCenterService.ApplyCommandCenterPluginPatch(patch);
            else
