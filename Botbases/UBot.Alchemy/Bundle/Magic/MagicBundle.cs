...
internal class MagicBundle : IAlchemyBundle
{
    ...
    public void Run<T>(T engineConfig)
    {
        ...
        foreach (var stone in config.MagicStones.Where(i => i.Key.Amount > 0))
        {
            ...
            var targetValue = current?.Record?.GetMaxValue() ?? stone.Value.GetMaxValue();
            if (
                config.TargetValues != null
                && config.TargetValues.TryGetValue(stone.Value.Group, out var configuredTargetValue)
                && configuredTargetValue > 0
            )
            {
                targetValue = (ushort)Math.Min((uint)targetValue, configuredTargetValue);
            }

            if (targetValue == 0)
                targetValue = stone.Value.GetMaxValue();

            Log.Debug($"[Alchemy] MaxStat: {targetValue}");

            if (current == null || current.Value < targetValue)
            {
